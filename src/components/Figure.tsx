    ...
    <figure style={{ ...WRAPPER_BY_SPAN[span], padding: '0 32px' }}>
      {showPlaceholder ? (
        <div style={{
          ...
          <div style={{ position: 'absolute', top: 18, left: 22, fontFamily: 'var(--font-primary)', fontSize: '12px', fontWeight: 400, color: tint, letterSpacing: 0.5, opacity: 0.8 }}>
            {alt ? `PLACEHOLDER · ${alt.toUpperCase()}` : 'PLACEHOLDER · IMAGE'}
          </div>
          {fig && (
            <div style={{ position: 'absolute', bottom: 20, right: 22, fontFamily: 'var(--font-primary)', fontSize: '12px', fontWeight: 400, color: tint, letterSpacing: 0.6, opacity: 0.7 }}>
              FIG. {fig}
            </div>
          )}
        </div>
